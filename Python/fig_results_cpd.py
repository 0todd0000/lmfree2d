...
R          = [lm.read_csv(os.path.join(dirREPO, 'Data', name, 'contours_sr.csv'))   for name in names]
...
for ax,r,template in zip(AX.flatten(), R, templates):
	for ii,rr in enumerate(r):
		if ii==template:
			ax.scatter(rr[:,0], rr[:,1], c=[c0], s=30, zorder=50)
		else:
			ax.scatter(rr[:,0], rr[:,1], c=[c1], s=5, zorder=0)
...
#(2) Save figure:
fnamePDF  = os.path.join(dirREPO, 'Figures', 'results_cpd.pdf')
